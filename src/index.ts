  ...
  async ({ address, blockNumber }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `ETH balance for ${address} at block "${
            blockNumber || "latest"
...
server.tool("get-block-number", "Get the latest block number", async ({}) => {
  ...
  if (!rpcResult || rpcResult.error) {
    return {
      content: [
        {
          type: "text",
          text: `Failed to retrieve latest block: ${
            rpcResult?.error?.message || "Unknown error"
          }`,
        },
      ],
  ...
  return {
    content: [
      {
        type: "text",
        text: `Latest block number: ${formatNumber(latestBlock)} (hex: ${
          rpcResult.result
        })`,
      },
    ],
  ...
  if (!rpcResult || rpcResult.error) {
    return {
      content: [
        {
          type: "text",
          text: `Failed to retrieve latest gas price: ${
            rpcResult?.error?.message || "Unknown error"
          }`,
        },
      ],
    ...
    if (!filterCreation || filterCreation.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to create filter: ${
              filterCreation?.error?.message || "Unknown error"
            }`,
          },
        ],
      };
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to create block filter: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to create new pending transcation filter: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to get filter changes: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to get logs for filter ID ${filterId}: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to submit PoW solution: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to fetch client version: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
    ...
    if (!rpcResult || rpcResult.error) {
      return {
        content: [
          {
            type: "text",
            text: `Failed to compute SHA3 hash: ${
              rpcResult?.error?.message || "Unknown error"
            }`,
          },
        ],
